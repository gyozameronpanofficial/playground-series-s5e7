...
### 成功手法（最終推奨）⭐
**統合版（Complex Hybrid Integration）**
```python
# 実装場所: src/phases/phase3_hybrid_integration.py
# 特徴量: 17個（心理学+Target Encoding+擬似ラベリング）
# 手法: Phase 2b + フェーズ1+2 統合 + sample_weight対応
# 結果: CV 0.976404 (全実装中最高), PB 0.975708（GM同値）
# 戦略: Private LBシェイクアップで攻めの戦略
```
...
### 代替手法（保守的選択肢）
**Phase 2b（高度Target Encoding）**: PB確実性重視の場合
```bash
...
2. **フェーズ1+2単体**: 擬似ラベリング効果が限定的
3. **sample_weight未対応**: 統合版の旧バージョン
