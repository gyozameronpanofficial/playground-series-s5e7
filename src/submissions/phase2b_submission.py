...
def main():
    """メイン実行関数"""
    print("=== Phase 2b 提出ファイル作成 ===")
    
    # 1. データ読み込み
    print("1. Phase 2b特徴量データ読み込み中...")
    try:
        train_data = pd.read_csv('/Users/osawa/kaggle/playground-series-s5e7/data/processed/phase2b_train_features.csv')
        test_data = pd.read_csv('/Users/osawa/kaggle/playground-series-s5e7/data/processed/phase2b_test_features.csv')
    ...
    # 3. モデル訓練
    print("3. Phase 2bモデル訓練中...")
    final_model = create_phase2b_final_model()
    ...
    # 保存
    submission_path = '/Users/osawa/kaggle/playground-series-s5e7/submissions/phase2b_target_encoding_submission.csv'
    submission_df.to_csv(submission_path, index=False)
    
    print(f"\\n✅ Phase 2b提出ファイル作成完了!")
    print(f"   ファイル: {submission_path}")
    print(f"   CVスコア: 0.968905 (期待値未達)")
    print(f"   備考: Phase 2aより微改善、CV-PB gapでの実際効果を確認")
